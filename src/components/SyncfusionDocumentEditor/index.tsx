...
class SynfDocumentEditor extends AppComponentListBase<IProps, IStates> {
  ...
  actionSaveDocx = () => {
    this.rteRef.current.documentEditor.keyDown = (args: DocumentEditorKeyDownEventArgs) => {
      const keyCode: number = args.event.which || args.event.keyCode
      const isCtrlKey: boolean = args.event.ctrlKey || args.event.metaKey ? true : keyCode === 17 ? true : false
      const isAltKey: boolean = args.event.altKey ? args.event.altKey : keyCode === 18 ? true : false
  ...
  loadDefault = async () => {
    const defaultDocument = this.props.value ?? notData
