...
const { activeStatus, includeExpired } = AppConsts
type Props = {
  handleSearch: any
  amenityStore: AmenityStore
}

const FilterMonthlyPackage = (props: Props) => {
  React.useEffect(() => {
    searchAmenity('')
  }, [])
  ...
  return (
    <Row gutter={[16, 8]}>
      ...
      <Col sm={{ span: 3, offset: 0 }}>
        <label>{L('FILTER_IS_INCLUDE_EXPIRED')}</label>
        <Select allowClear className="w-100" onChange={(value) => props.handleSearch('isIncludeExpired', value)}>
          {renderOptions(includeExpired)}
        </Select>
      </Col>
    </Row>
  )
...
export default FilterMonthlyPackage
