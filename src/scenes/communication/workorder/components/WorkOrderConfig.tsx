...
const WorkOrderConfig = () => {
  return isGranted(appPermissions.workorderConfiguration.page) ? (
    <div className="card-container sla-container">
      <Tabs type="card">
        <TabPane tab={L('STATUS')} key="1">
          <WfStatus wfStatusStore={new WfStatusStore()} moduleId={moduleIds.workOrder} />
        </TabPane>
        <TabPane tab={L('PRIORITY')} key="2">
          <WfPriority wfPriorityStore={new WfPriorityStore()} moduleId={moduleIds.workOrder} />
        </TabPane>
        <TabPane tab={L('ROLE')} key="3">
          <WfRole wfRoleStore={new WfRoleStore()} moduleId={moduleIds.workOrder} />
        </TabPane>
        <TabPane tab={L('TRACKER')} key="4">
          <WfTracker wfTrackerStore={new WfTrackerStore()} moduleId={moduleIds.workOrder} />
        </TabPane>
        <TabPane tab={L('CUSTOM_FIELD')} key="5">
          <WfCustomField wfCustomFieldStore={new WfCustomFieldStore()} moduleId={moduleIds.workOrder} />
        </TabPane>
        ...
        <TabPane tab={L('SLA')} key="7">
          <SettingWorkflowSLA moduleId={moduleIds.workOrder} />
        </TabPane>
      </Tabs>
    </div>
  ) : (
