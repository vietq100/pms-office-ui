...
@inject(Stores.VisitorStore)
class ImportVisitorModal extends AppComponentListBase<Props, State> {
  constructor(props) {
    super(props)
    this.state = {
      file: null,
  ...
  handleUpload = async () => {
    try {
      ...
      if (!file) {
        ...
        return notifyError(L('ERROR'), L('VISITOR_IMPORT_FILE_REQUIRED'))
      }
  ...
  render(): React.ReactNode {
    ...
    return (
      <Modal
        open={visible}
        destroyOnClose
        title={L('VISITOR_IMPORT_MODAL_TITLE')}
        ...
        <Row gutter={[8, 16]}>
          ...
          <Col span={24} className="mb-2 mt-1">
            <Button
              ...
              style={{ width: '100%' }}
              shape="round"
              onClick={this.props?.visitorStore.downloadTemplate}>
...
export default withRouter(ImportVisitorModal)
