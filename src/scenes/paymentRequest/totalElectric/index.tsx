...
@inject(Stores.SessionStore, Stores.TotalElectricMeterStore)
@observer
class TotalElectricMeter extends AppComponentListBase<IProps, IState> {
  state = {
    maxResultCount: 10,
    skipCount: 0,
    listStatus: [],
  ...
  getAll = async () => {
    await this.props.totalElectricMeterStore.getAll({
  ...
  deleteItem = async (id: number) => {
    ...
    confirm({
      ...
      cancelText: L('BTN_NO'),
      onOk: async () => {
        await self.props.totalElectricMeterStore.delete(id)
  ...
  copyItem = (id: number) => {
    this.props.navigate(portalLayouts.totalElectricMeterCreate.path, { state: { copyId: id } })
  }
  ...
  gotoDetail = (id?) => {
    ...
    id
      ? window.open(portalLayouts.totalElectricMeterDetail.path.replace(':id', id), '_blank')
      : navigate(portalLayouts.totalElectricMeterCreate.path)
  }

  public render() {
    const {
      totalElectricMeterStore: { dataTable }
    } = this.props
...
export default withRouter(TotalElectricMeter)
