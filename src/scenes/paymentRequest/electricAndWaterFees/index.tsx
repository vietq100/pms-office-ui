...
@inject(Stores.SessionStore, Stores.ElectricAndWaterFeeStore)
@observer
class ElectricAndWaterFees extends AppComponentListBase<IProps, IState> {
  state = {
  ...
  getAll = async () => {
    await this.props.electricAndWaterFeeStore.getList({
      ...this.state.filters
    })
  }
  ...
  deleteItem = async (id: number) => {
    ...
    confirm({
      ...
      cancelText: L('BTN_NO'),
      onOk: async () => {
        await self.props.electricAndWaterFeeStore.delete(id)
      }
    })
  }
  ...
  gotoDetail = (id?) => {
    ...
    id
      ? window.open(portalLayouts.electricAndWaterFeeDetail.path.replace(':id', id), '_blank')
      : navigate(portalLayouts.electricAndWaterFeeCreate.path)
  }

  public render() {
    const {
      electricAndWaterFeeStore: { dataTable }
    } = this.props
...
export default withRouter(ElectricAndWaterFees)
