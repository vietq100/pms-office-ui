...
export interface IDeliveryFormProps {
  ...
  params: any
  fileStore: FileStore
  electricAndWaterFeeStore: ElectricAndWaterFeeStore
  sessionStore: SessionStore
}

@inject(Stores.ElectricAndWaterFeeStore, Stores.FileStore, Stores.SessionStore)
@observer
class ElectricAndWaterFeeDetail extends AppComponentBase<IDeliveryFormProps> {
  ...
  getDetail = async (id?) => {
    if (!id) {
      await this.props.electricAndWaterFeeStore.initRecord()
      ...
      await this.props.electricAndWaterFeeStore.get(id)

      await this.props.electricAndWaterFeeStore.getListRequestHistory({
        id: this.props.electricAndWaterFeeStore.dataDetail?.id,
        requestTypeId: ticketRequestTypeEnum.WATER_AND_ELECTRIC_FEE
      })
      this.setState({
        allowApprovalOrReject: this.props.electricAndWaterFeeStore.listRequestHistory.some(
          (item) => item?.canSendRequest === true
        )
      })
      this.formHistoryAppropval.current.setFieldsValue({
        histories: [...this.props.electricAndWaterFeeStore.listRequestHistory].map((item) => ({
          ...item,
          lastModificationTime: item.lastModificationTime ? dayjs(item.lastModificationTime) : null
        }))
      })
      this.setState({
        idDocument: this.props.electricAndWaterFeeStore.dataDetail?.uniqueId,
        dataElectricFee: this.props.electricAndWaterFeeStore.dataDetail
      })
      this.setState((prevState) => ({
        listPeriod: addItemToList(prevState.listPeriod, this.props.electricAndWaterFeeStore.dataDetail?.feePackage)
      }))
    }
    if (this.formRef.current) {
      this.formRef.current.setFieldsValue({
        ...this.props.electricAndWaterFeeStore.dataDetail
      })
    }
  }
  ...
  handleApprovalOrReject = async (message: string, typeModalAproval: string) => {
    ...
    if (typeModalAproval === statusApproval.APPROVAL) {
      await this.props.electricAndWaterFeeStore.sendApproval({ ...body, statusId: ticketRequestStatusEnum.Approval })
    } else {
      await this.props.electricAndWaterFeeStore.sendApproval({ ...body, statusId: ticketRequestStatusEnum.Rejected })
    }
  ...
  onSave = async () => {
    ...
    form.validateFields().then(async (values: any) => {
      await this.props.electricAndWaterFeeStore.createOrUpdate({ ...values, id }, this.state.files)
      await this.props.navigate(portalLayouts.electricAndWaterFee.path)
    })
  }
  ...
  onCancel = () => {
    ...
    confirm({
      ...
      cancelText: L('BTN_NO'),
      onOk: () => {
        navigate(portalLayouts.electricAndWaterFee.path)
      }
    })
    return
  }
  ...
  getDataWaterFee = async (id) => {
    if (!id) return

    const dataWater = await this.props.electricAndWaterFeeStore.getDetailWaterByFeePackageId(id)

    this.formRef.current.setFieldValue('dateOfCreationOfWater', dayjs(dataWater?.dateOfRecording))
    this.formRef.current.setFieldValue('waterFromIndex', dataWater?.fromIndex)
    this.formRef.current.setFieldValue('waterToIndex', dataWater?.toIndex)
    this.formRef.current.setFieldValue('waterTotalUnit', dataWater?.total)
    this.formRef.current.setFieldValue('wasteWaterUnitPrice', dataWater?.wasteWaterUnitPrice)
    this.formRef.current.setFieldValue('wasteWaterTotalAmount', dataWater?.wasteWaterTotalAmount)
    this.formRef.current.setFieldValue('wasteWaterVATAmount', dataWater?.wasteWaterVATAmount)
    this.formRef.current.setFieldValue('waterTotalAmount', dataWater?.totalAmount)
  }

  getDataElectricFee = async (id) => {
    if (!id) return

    const dataElectric = await this.props.electricAndWaterFeeStore.calculateElectricityFee(id)
    this.setState({ dataElectricFee: dataElectric })
  }
  onRemoveFile = (file) => {
    ...
    return (
      <Row>
        <Col sm={{ span: 24, offset: 0 }}>
          ...
          {this.props.electricAndWaterFeeStore?.dataDetail?.id && (
            <ReactToPrint
              trigger={() => (
              ...
              documentTitle="electric-and-water-fee"
  ...
  public render() {
    ...
    const { dataElectricFee } = this.state
    return (
        ...
        <Card bordered={false} style={{ minHeight: 750 }} ref={this.printRef}>
          <Form
            ...
            <Row gutter={[8, 4]}>
              ...
              <Col sm={{ span: 24, offset: 0 }} className="mt-2">
                <label className="title-detail">{L('WATER_FEE_INFO')}</label>
              </Col>

              <Col sm={{ span: 24, offset: 0 }} className="mt-2">
                <label className="title-detail">{L('WATER_CONST')}</label>
              </Col>
              <Col sm={{ span: 8, offset: 0 }}>
                <Form.Item
                  label={L('METER_PERIOD')}
                  {...formVerticalLayout}
                  name="waterFeePackageId"
              ...
              <Col sm={{ span: 8, offset: 0 }}>
                <Form.Item name="dateOfCreationOfWater" label={L('RECORD_DATE')}>
                  <DatePicker format={dateFormat} style={{ width: '100%' }} disabled />
                </Form.Item>
              </Col>
              <Col sm={{ span: 8, offset: 0 }} />
              <Col sm={{ span: 8, offset: 0 }}>
                <FormNumber label={L('START_RECORD')} disabled name="waterFromIndex" />
              </Col>
              <Col sm={{ span: 8, offset: 0 }}>
                <FormNumber label={L('END_RECORD')} disabled name="waterToIndex" />
              </Col>
              <Col sm={{ span: 8, offset: 0 }}>
                <FormNumber label={L('QUANTITY')} disabled name="waterTotalUnit" />
              </Col>
              <Col sm={{ span: 24, offset: 0 }} className="mt-2">
                <label className="title-detail">{L('WASTE_WATER_FEE')}</label>
              </Col>

              <Col sm={{ span: 8, offset: 0 }}>
                <FormNumber label={L('AMOUNT')} disabled name="wasteWaterUnitPrice" />
              </Col>
              <Col sm={{ span: 8, offset: 0 }}>
                <FormNumber label={L('TO_PRICE')} disabled name="wasteWaterTotalAmount" />
              </Col>
              <Col sm={{ span: 8, offset: 0 }}>
                <FormNumber label={L('GTGT_TAX')} disabled name="wasteWaterVATAmount" />
              </Col>
              <Col sm={{ span: 24, offset: 0 }} className="mt-2">
                <label className="title-detail">{L('TOTAL_AMOUNT')}</label>
              </Col>
              <Col sm={{ span: 8, offset: 0 }}>
                <FormNumber label={L('TOTAL_AMOUNT_PAYMENT')} disabled name="waterTotalAmount" />
              </Col>

              <>
                <Col sm={{ span: 24, offset: 0 }} className="mt-2">
                  <label className="title-detail">{L('ELECTRIC_FEE_INFO')}</label>
                </Col>
                <Col sm={{ span: 6, offset: 0 }}>
                  <Form.Item
                    label={L('METER_PERIOD')}
                    {...formVerticalLayout}
                    name="electricityFeePackageId"
                    rules={rules.required}>
                    <Select
                      showSearch
                      disabled={!isCreate}
                      ...
                      filterOption={filterOptions}
                      className="full-width"
                      onChange={this.getDataElectricFee}
                      onSearch={this.getLsitPeriod}>
                      {this.renderOptions(this.state.listPeriod)}
                    </Select>
                  </Form.Item>
                ...
                <Col sm={{ span: 6, offset: 0 }}>
                  <Form.Item name="dateOfCreationOfElectricity" label={L('RECORD_DATE')}>
                    <DatePicker format={dateFormat} style={{ width: '100%' }} disabled />
                  </Form.Item>
                </Col>
                <Col sm={{ span: 8, offset: 0 }} />
                <Col sm={{ span: 24 }}>
                  <Spreadsheet form={this.formRef} initData={dataElectricFee} />
                </Col>
...
export default withRouter(ElectricAndWaterFeeDetail)
