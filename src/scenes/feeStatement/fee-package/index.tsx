  ...
  public render() {
    ...
    const filterComponent = (
      <Row gutter={[16, 8]}>
        <Col sm={{ span: 6, offset: 0 }}>
          ...
          <Input.Search onSearch={this.handleSearch} placeholder={this.L('PACKAGE_FEE_NAME')} />
        </Col>
        <Col sm={{ span: 6, offset: 0 }}>
    ...
    return (
      <div className="fee-period-container">
        {/* <Filter
          title={this.L('FILTER')}
          handleRefresh={() => this.getAll({ skipCount: 0 })}>
          <Row gutter={[16, 8]}>
            <Col sm={{ span: 6, offset: 0 }}>
              <label>{this.L('FILTER_KEYWORD')}</label>
              <Input.Search
                onSearch={this.handleSearch}
                placeholder={this.L('PACKAGE_FEE_NAME')}
              />
            </Col>
            <Col sm={{ span: 6, offset: 0 }}>
              <label>{this.L('FILTER_PERIOD')}</label>
              <InputNumber
                style={{ width: '100%' }}
                onChange={debounce(this.handlePeriodSearch, 300)}
              />
            </Col>
          </Row>
        </Filter> */}
