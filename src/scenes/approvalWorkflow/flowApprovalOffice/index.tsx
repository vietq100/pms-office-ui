...
import FlowTenant2OperatorModal from './components/FlowTenant2OperatorModal'
import FlowOperator2TenantModal from './components/FlowOperator2TenantModal'
...
@observer
class FlowOperator2TenantPage extends AppComponentListBase<Props, State> {
  formRef: any = React.createRef()

  state = {
    maxResultCount: 10,
    skipCount: 0,
    isShowModalTenant: false,
    isShowModalBQL: false,
    idDetail: undefined,
    filters: { type: typeFlowOperator2Object.Operator2Tenant, isActive: 'true' }
  }
  ...
  generateRequestConfig = async () => {
    await this.props.flowApprovalOfficeStore
      .generateRequestConfig({ typeId: positionTypeEnum.Tenant })
      .then(() => this.getAll())
  }
  ...
  gotoDetail = (record) => {
    ...
    if (record?.approvalOrder === positionTypeEnum.Tenant) {
      this.setState({ isShowModalTenant: true })
    } else {
  ...
  onCloseModal = () => {
    this.setState({ idDetail: undefined })
    this.setState({ isShowModalTenant: false })
...
export default withRouter(FlowOperator2TenantPage)
