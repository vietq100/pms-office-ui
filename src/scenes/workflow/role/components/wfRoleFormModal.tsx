...
class WfRoleFormModal extends AppComponentBase<ICreateOrUpdateWfStatusProps> {
  state = {
    confirmDirty: false
  }
        ...
        <Form ref={this.props.formRef} layout={'vertical'} validateMessages={validateMessages} size="middle">
          <Row gutter={[16, 0]}>
            <Col sm={{ span: 24, offset: 0 }}>
              <Form.Item label={L('WF_ROLE_NAME')} {...formVerticalLayout} name="names" rules={rules.names}>
                <MultiLanguageInput />
              </Form.Item>
            </Col>
            <Col sm={{ span: 12, offset: 0 }}>
              <Form.Item {...formVerticalLayout} name="isDefault" valuePropName="checked">
                <Checkbox>{L('WF_ROLE_IS_DEFAULT')} </Checkbox>
              </Form.Item>
            </Col>
            <Col sm={{ span: 12, offset: 0 }}>
              <Form.Item {...formVerticalLayout} name="isActive" valuePropName="checked">
                <Checkbox>{L('WF_ROLE_IS_ACTIVE')} </Checkbox>
              </Form.Item>
            </Col>
            {!this.props.moduleId && (
              <Col sm={{ span: 24, offset: 0 }}>
...
export default WfRoleFormModal
