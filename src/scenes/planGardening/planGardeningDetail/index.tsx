...
import { Card, Col, Form, Input, Row, Select, DatePicker, Tabs, Button, Modal, InputNumber, Table } from 'antd'
...
import columnNote from '@scenes/planSanitation/planSanitationDetail/columnNote'
...
export interface IPlanSanitationProps {
  params: any
  navigate: any
...
@observer
class PlanSanitationDetail extends AppComponentBase<IPlanSanitationProps> {
  state = {
    isDirty: false,
    userId: undefined,
    uniqueId: '',
  ...
  getListFloor = debounce(async (keyword?) => {
    const listFloor = await handoverService.getListFloor({ keyword })
  ...
  getDetail = async (id?) => {
    ...
    if (servicePlanStore.editPlanSanitation?.recurring) {
      this.formRef.current.setFieldsValue({
        ...
        // recurringStartTime: dayjs('01/01/2021 '
        // + (servicePlanStore.editPlanSanitation.recurring.startTime || '00:00')),
    ...
    form.validateFields().then(async (values: any) => {
      ...
      if (servicePlanStore.editPlanSanitation?.id) {
        await servicePlanStore.update(
          {
            ...servicePlanStore.editPlanSanitation,
            type: servicePlanEnum.GARDENING,
            ...values,
      ...
      form.resetFields()
      this.props.navigate(portalLayouts.planGardening.path)
    })
  }
      ...
      <Row gutter={[16, 20]}>
        <Card className="full-width">
          <Row gutter={[16, 0]}>
            ...
            <Col sm={{ span: 24, offset: 0 }}>
              <Form.Item label={L('PLANED_SANITATION_FLOORS')} {...formVerticalLayout} name="floorIds">
                <Select
                  ...
                  allowClear
                  filterOption={false}
                  onSearch={this.getListFloor}
                  onClear={() => this.getListFloor('')}
            ...
            {/* <Col sm={{ span: 24, offset: 0 }}>
              <Form.Item label={L('PLANED_SANITATION_NOTE')} {...formVerticalLayout} name="description">
...
export default withRouter(PlanSanitationDetail)
