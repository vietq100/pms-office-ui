...
class PlanSanitation extends AppComponentListBase<IContactProfileProps, IContractProfileState> {
  state = {
    ...
    skipCount: 0,
    assigners: [] as any,
    filters: { firmId: undefined, isActive: 'true', type: servicePlanEnum.GARDENING }
  }
  ...
  gotoDetail = async (id?) => {
    ...
    if (id) {
      navigate(portalLayouts.planGardeningEdit.path.replace(':id', id))
    } else {
      navigate(portalLayouts.planGardeningCreate.path)
    }
  }
