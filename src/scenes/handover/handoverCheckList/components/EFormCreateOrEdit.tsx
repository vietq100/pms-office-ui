    ...
    const onUpdateForm = async () => {
      return form.validateFields().then(async () => {
        ...
        if (eFormStore.editEForm.id) {
          ...
          navigate(portalLayouts.handoverCheckListEdit.path.replace(':id', eFormStore.editEForm?.id))
        }
      })
    }
      ...
      <div className="w-100">
          ...
          <Card className="w-100 d-flex justify-content-center align-items-center">
            <Tooltip placement="bottom" title={L('BTN_ADD_QUESTION')}>
              <Button
                ...
                disabled={eFormStore.editEForm?.statusId === efromStatusPublic.PUBLISHED}>
        ...
        <ActionFooter show={true}>
          ...
          <Button
            ...
            onClick={() => navigate(portalLayouts.handoverChecklistTemplate.path)}>
            {L('BTN_BACK_TO_LIST')}
          </Button>
        </ActionFooter>
