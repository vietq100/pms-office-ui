...
class ModalCreateRequest extends AppComponentBase<IProps> {
  ...
  handlePackageSearch = async (value) => {
    const packages = await packageFeeService.filter({ keyword: value })
  ...
  handleSubmit = () => {
    ...
    form.validateFields().then((values: any) => {
      this.setState({ totalCompany: values.companyIds?.length })
      this.checkRequestIsValid(values)
    })
  }

  checkRequestIsValid = async (dataCheck) => {
    await this.props.vehicleRegistrationFormStore.checkRequestIsValid(dataCheck)

    if (this.props.vehicleRegistrationFormStore.listIdCompanyError?.length > 0) {
      this.setState({ isAllowCompanyExist: true })
    } else {
      this.setState({ isAllowCompanyExist: false })
      this.createRequest(dataCheck)
      this.props.onCancel(true)
    }
  }

  createRequest = async (dataCreate?: any) => {
    const form = this.formRef.current
    if (dataCreate) {
      await this.props.vehicleRegistrationFormStore.createRequest(dataCreate)
    } else {
      form.validateFields().then(async (values: any) => {
        await this.props.vehicleRegistrationFormStore.createRequest(values)
      })
    }
          ...
          title={
            <div className="d-flex justify-content-center w-100">
              {!this.state.isAllowCompanyExist ? (
                L('TITLE_CREATE_REQUEST_VEHICE')
              ) : (
                <>
                  {L('TITLE_CREATE_REQUEST_VEHICE')} {this.state.namePackageNotify}
                </>
              )}
            </div>
          }
            ...
            <>
              {!this.state.isAllowCompanyExist ? (
                ...
                <div className="d-flex justify-content-center">
                  ...
                  <Button key="submit" type="primary" onClick={() => this.createRequest()}>
                    {L('BTN_OK')}
                  </Button>
                </div>
              )}
            ...
            <Col sm={{ span: 24, offset: 0 }}>
              <p>
                {L(
                  ...
                  this.props.vehicleRegistrationFormStore.listIdCompanyError?.length
                )}
              </p>
            </Col>
          </Row>
