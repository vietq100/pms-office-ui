...
@inject(Stores.ProjectStore, Stores.BuildingStore, Stores.FileStore, Stores.SessionStore)
@observer
class ProjectSetting extends AppComponentBase<IProjectsProps, IProjectsState> {
  ...
  async componentDidMount() {
    await this.getProjectBankSetting(this.props.sessionStore.projectId)
  ...
  onSaveBankSetting = () => {
    ...
    form.validateFields().then(async (values: any) => {
      if (this.props.sessionStore.projectId) {
        const arrBankList = [...values.bankOBA, ...values.bankDBA]
        await this.props.projectStore.updateBank(arrBankList, this.props.sessionStore.projectId)
      }
    })
  }
      ...
      <Row>
        <Col sm={{ span: 24, offset: 0 }}>
          <Button
            type="primary"
            disabled={
              !isGranted(appPermissions.paymentSetting.update) || !isGranted(appPermissions.paymentSetting.create)
            }
