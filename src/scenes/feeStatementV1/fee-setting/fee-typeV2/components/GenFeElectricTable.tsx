...
import { Button, Form, InputNumber, Col, Row, Input, Select } from 'antd'
...
@inject()
@observer
class GenFeElectricTable extends AppComponentBase<Props, State> {
  maxResultCount = 10
  ...
  async componentDidMount() {
    const description = this.props.dataConfig?.feeElectricityConfigurations
      ? this.props.dataConfig?.feeElectricityConfigurations
    ...
    this.props.formEletricConfig.current.setFieldsValue({
      ...
      feeElectricityConfigurations: this.props.dataConfig?.feeElectricityConfigurations
        ? this.props.dataConfig.feeElectricityConfigurations.filter((item) => item.type === this.props.purposeUsing)
        : []
    })
  }
  onChange = async (action) => {
    const arrConfig =
      this.props.formEletricConfig.current.getFieldsValue(['feeElectricityConfigurations'])
        .feeElectricityConfigurations ?? []
  ...
  checkDuplicate = (value: any) => {
    const checkData = this.props.formEletricConfig.current
      .getFieldsValue()
      .feeElectricityConfigurations.filter((item) => item.indexNumber === value)
      ...
      <>
        <Form validateMessages={validateMessages} ref={formEletricConfig} layout={'vertical'} size="middle">
          ...
          <Row gutter={[8, 8]} style={{ marginBottom: 10 }} className="custom-col-fee-config">
            ...
            <Col span={4}>{L('FEE_CONFIG_INDEX')}</Col>
            {this.props.purposeUsing === purposeUsing.residential && <Col span={4}>{L('FEE_CONFIG_FROM')}</Col>}
            {this.props.purposeUsing === purposeUsing.residential && <Col span={4}>{L('FEE_CONFIG_TO')}</Col>}
            <Col span={4}>{L('FEE_CONFIG_AMOUNT')}</Col>
            <Col span={4}>{L('FEE_CONFIG_VAT')}</Col>

            <Col span={3}>{L('FEE_CONFIG_ACTION')}</Col>
          </Row>
          <Form.List name="feeElectricityConfigurations">
            {(fields, { add, remove }) => (
              <>
                  ...
                  <Row key={key} gutter={[8, 8]}>
                    ...
                    {this.props.purposeUsing === purposeUsing.residential && (
                      <Col span={4}>
                        <Form.Item
                          {...restField}
                          messageVariables={{ label: L('FEE_CONFIG_FROM') }}
                          name={[name, 'fromIndex']}
                          rules={[{ required: true }]}
                          // cộng 1 để cho fee tiếp theo cộng 1 đơn vị
                          initialValue={this.state.fromIndexValueDefult + 1}>
                          <InputNumber
                            min={0}
                            disabled
                    ...
                    {this.props.purposeUsing === purposeUsing.residential && (
                      <Col span={4}>
                        <Form.Item
                          {...restField}
                          messageVariables={{ label: L('FEE_CONFIG_TO') }}
                          name={[name, 'toIndex']}
                          ...
                          <InputNumber
                            ...
                            // onChange={(value) =>
                            //   this.setState({ fromIndexValueDefult: value })
                            // }
                            formatter={(value) => inputNumberFormatter(value)}
                            className="full-width"
                          />
...
export default GenFeElectricTable
