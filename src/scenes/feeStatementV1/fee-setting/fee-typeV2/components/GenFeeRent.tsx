...
@observer
class GenFeeRent extends AppComponentBase<Props, State> {
  maxResultCount = 10
  ...
  async componentDidMount() {
    ...
    const description = this.props.dataConfig?.feeRentConfigurations
      ? this.props.dataConfig?.feeRentConfigurations.map((item) => {
    ...
    this.props.formManagement.current.setFieldsValue({
      ...
      feeRentConfigurations: this.props.dataConfig?.feeRentConfigurations
        ? this.props.dataConfig.feeRentConfigurations
        : [],
      contentTemplate: this.props.dataConfig?.contentTemplate ? this.props.dataConfig.contentTemplate : ''
    })

    if (this.props.dataConfig?.feeRentConfigurations?.length > 0) {
  ...
  checkDuplicate = (value: any) => {
    const checkData = this.props.formManagement.current
      .getFieldsValue()
      .feeRentConfigurations.filter((item) => item.unitTypeId === value)
    ...
    return (
      <>
        <Form validateMessages={validateMessages} ref={formManagement} layout={'vertical'} size="middle">
          ...
          <Form.List name="feeRentConfigurations">
            {(fields, { add, remove }) => (
              <>
