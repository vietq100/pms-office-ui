...
const SaleManagement = inject(Stores.SaleAndLeaseStore)(
  observer((props: Props) => {
    ...
    // React.useEffect(() => {
    // isGranted(appPermissions.enquiry.page) && props.saleAndLeaseStore.getAllSale(filter)
    // isGranted(appPermissions.enquiry.page) && props.saleAndLeaseStore.getListStatus()
    // isGranted(appPermissions.enquiry.page) &&
    //   props.saleAndLeaseStore.getSaleOverview({ ...filter, enquiryTypeId: enquiryType.sale })
    ...
    const gotoDetail = async (id?) => {
      id ? navigate(portalLayouts.saleDetail.path.replace(':id', id)) : navigate(portalLayouts.saleCreate.path)
    }
    ...
    const column = getColumns({
      title: L('SALE_DESCRIPTION'),
      dataIndex: 'description',
      key: 'description',
      ellipsis: true,
      width: '20%',
      render: (description: string, item: any) => {
    ...
    return isGranted(appPermissions.enquiry.page) ? (
      <>
        <OverViewBar
          data={props.saleAndLeaseStore.saleOverview}
          handleClickItem={() => {
        ...
        <DataTable
          ...
          onRefresh={() => props.saleAndLeaseStore.getAllSale(filter)}
          title={L('SALE_LIST')}
