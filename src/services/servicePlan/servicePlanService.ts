...
class ServicePlanService {
  ...
  public async activateOrDeactivate(id: number, isActive: boolean) {
    if (isActive) {
      return http.post('/api/services/app/ServicePlans/Active', { id }, { params: { isActive } })
    }
    return http.post('/api/services/app/ServicePlans/Active', { id }, { params: { isActive } })
  }
  ...
  public async exportServicePlan(params: any): Promise<any> {
    const res = await http.get('api/Export/ServicePlan', {
      params,
      responseType: 'blob'
    })
    if (params?.type === servicePlanEnum.SANITTION) {
      downloadFile(res.data, `Plan_Sanitation_${renderDateTime(dayjs())}.xlsx`)
    }

    if (params?.type === servicePlanEnum.GARDENING) {
      downloadFile(res.data, `Plan_Gardening_${renderDateTime(dayjs())}.xlsx`)
    }
  }
}
